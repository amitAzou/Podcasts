...
jest.mock('../../../ models/fileModel', () => ({
  ...
  getBestPodcastsFromDataBase: (number) => number === 1 ? [1] : []
}))
...
describe('Component Tests:', () => {
  ...
  describe('Search podcast tests:', () => {
    it('It should return 404 when GET request is called with a un-existing keyword of title or author', async () => {
  ...
  describe('Get best podcasts tests:', () => {
    it('It should return 400 when GET request is called with a invalid number type', async () => {
      await supertest(app).get('/podcast/best/wrong').expect(400)
    })

    it('It should return 200 when GET podcast is called with a valid number type', async () => {
